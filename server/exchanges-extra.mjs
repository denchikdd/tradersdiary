...
export function createExtraAdapter(exchange,credentials,options={},fetchImpl=fetch){
  ...
  async function asterPrivate(paths,params={},host){let last;for(const path of paths){try{return await request(path,params,host);}catch(e){last=e;}}throw last;}
  async function verify(){
    if(exchange==='Aster'){if(!credentials.apiKey||!credentials.secret)throw new ExchangeError('Переподключите Aster с read-only API Key и API Secret. Публичный адрес с Account Privacy не раскрывает баланс и историю.');await asterPrivate(['/fapi/v3/account','/fapi/v2/account','/fapi/v2/balance']);return;}
