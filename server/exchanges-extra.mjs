...
export function createExtraAdapter(exchange,credentials,options={},fetchImpl=fetch){
  ...
  async function verify(){
    if(exchange==='Aster'){const result=await asterRpc('aster_getBalance',[credentials.address,'latest']);if(!result?.address)throw new ExchangeError('Aster не нашёл аккаунт по этому публичному адресу.');return;}
  ...
  async function snapshot(){
    if(exchange==='Aster'){const a=await asterRpc('aster_getBalance',[credentials.address,'latest']),assets=a.perpAssets||[];return {equityUsd:decimal(assets.reduce((n,v)=>['USDT','USDC','USD1'].includes(v.asset)?n+units(v.walletBalance||'0'):n,0n)),wallet:a,scope:'spot+perpetuals',warning:a.accountPrivacy==='enabled'?'В Aster включена приватность: часть публичной истории может быть скрыта.':null};}
  ...
  function streams(now){
    if(exchange==='Aster')return [{id:'rpc-futures',start:1772678119418,window:7*DAY},{id:'rpc-spot',start:1772678119418,window:7*DAY}];
  ...
  async function page(stream,start,end,cursor){let rows=[],next=null;
    if(exchange==='Aster'){const method=stream==='rpc-spot'?'aster_spotUserFills':'aster_userFills',r=await asterRpc(method,[credentials.address,null,start,end,'latest']);rows=r?.fills||[];return {events:rows.map((v,i)=>event(v,{id:`${v.time}:${v.symbol}:${v.side}:${i}`,time:v.time,market:stream==='rpc-spot'?'spot':'futures',symbol:v.symbol,currency:v.symbol?.endsWith('USDC')?'USDC':'USDT'})),next:null};}
