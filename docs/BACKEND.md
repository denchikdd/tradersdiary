...
Таблицы: connections (шифротекст), events (оригиналы и нормализованные события), jobs (курсоры, окна, повторы), snapshots (последний баланс/позиции), day_notes, note_images, settings, sessions, login_attempts. Страница и курсор сохраняются транзакционно. Уникальность: connection_id/stream/external_id. Потоки fills и ledger разделены, чтобы не посчитать комиссии дважды.

## Дневные заметки
Текст заметки привязан к календарной дате UTC. Ссылки отображаются кликабельными, но текст не интерпретируется как HTML. Скриншоты PNG/JPEG/WebP хранятся BLOB-данными в той же SQLite-базе, доступны только через авторизованный API и ограничены 5 МБ на файл и 12 файлами на день. Поэтому backup Railway Volume включает и заметки, и изображения.
